...
- The levels loop forever: level 1, level 2, back to level 1, and on until the ranger dies. Every ten cards moves to the next level in the loop.
- Each time the loop wraps back to level 1 a new, harder cycle begins. Enemies gain a flat 10 health and 0.05 speed per cycle, so the second pass through the knights starts them at 12, 14, and 16 health and the trolls at 18, 20, and 22, and it keeps climbing from there.
